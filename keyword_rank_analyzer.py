...
def results(session_id):
    ...
    try:
        ...
        if not analysis_data:
            # 检查是否分析正在进行中
            if session_id in analysis_results and not analysis_status.get(session_id, False):
                return redirect(url_for('progress', session_id=session_id))
            else:
...
def is_analysis_complete(session_id):
    """检查分析是否完成"""
    # 检查analysis_results目录中是否有对应的JSON文件和完成标记
    analysis_dir = os.path.join(os.path.dirname(__file__), 'analysis_results')
    ...
    return os.path.exists(json_file) and os.path.exists(latest_done_file)
