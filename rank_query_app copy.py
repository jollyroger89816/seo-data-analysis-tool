...
def get_latest_analysis_result():
    ...
    try:
        ...
        # 获取所有.done文件（表示分析完成）
        done_files = glob.glob(os.path.join(analysis_dir, '*.done'))
        
        if not done_files:
            logging.warning("没有找到完成的分析结果")
            return None
        
        # 按修改时间排序，获取最新的
        done_files.sort(key=os.path.getmtime, reverse=True)
        latest_done_file = done_files[0]
        
        # 获取对应的JSON文件
        json_file = latest_done_file.replace('.done', '.json')
        
        if not os.path.exists(json_file):
            logging.warning(f"对应的JSON文件不存在: {json_file}")
            return None
        ...
        logging.info(f"成功加载最新分析结果: {json_file}")
        return data
...
def get_all_analysis_reports():
    ...
    try:
        ...
        for done_file in done_files:
            json_file = done_file.replace('.done', '.json')
...
def get_analysis_result_by_timestamp(timestamp):
    ...
    try:
        ...
        if not os.path.exists(json_file):
            logging.warning(f"指定的分析结果文件不存在: {json_file}")
            return None
...
def get_analysis_info():
    ...
    try:
        ...
        # 获取目标下降词数量
        target_decline_count = len(comparative_analysis.get('target_decline_details_full', []))
        if target_decline_count == 0:
        ...
        return jsonify({
            ...
            'target_decline_count': target_decline_count,
            'category': comparative_analysis.get('category', ''),
            'date_range': comparative_analysis.get('date_range_current', ''),
            'common_decline_count': comparative_analysis.get('common_decline_ids_count', 0),
            'total_decline_count': comparative_analysis.get('total_decline_ids_count', 0)
        })
...
def get_report_info(timestamp):
    ...
    try:
        ...
        logging.error(f"获取指定报告信息失败: {str(e)}")
        return jsonify({
            'success': False,
...
def export_results():
    ...
    try:
        ...
        # 获取查询结果
        global query_progress_data
        if query_progress_data['session_id'] != session_id:
            return jsonify({'error': '无效的会话ID'})
        
        if query_progress_data['status'] != 'finished':
            return jsonify({'error': '排名查询未完成'})
        
        query_results = query_progress_data['results']
...
def get_filter_statistics():
    ...
    try:
        ...
        # 添加分布统计
        decline_amount_ranges = [
            {'range': '0-1000', 'count': len([x for x in decline_amounts if 0 <= x <= 1000])},
            {'range': '1001-5000', 'count': len([x for x in decline_amounts if 1001 <= x <= 5000])},
            {'range': '5001-10000', 'count': len([x for x in decline_amounts if 5001 <= x <= 10000])},
            {'range': '10001-50000', 'count': len([x for x in decline_amounts if 10001 <= x <= 50000])},
            {'range': '50000+', 'count': len([x for x in decline_amounts if x > 50000])}
        ]
        
        decline_rate_ranges = [
            {'range': '50-60%', 'count': len([x for x in decline_rates if 50 <= x <= 60])},
            {'range': '60-70%', 'count': len([x for x in decline_rates if 60 < x <= 70])},
            {'range': '70-80%', 'count': len([x for x in decline_rates if 70 < x <= 80])},
            {'range': '80-90%', 'count': len([x for x in decline_rates if 80 < x <= 90])},
            {'range': '90-100%', 'count': len([x for x in decline_rates if 90 < x <= 100])}
        ]
        
        stats['decline_amount_distribution'] = decline_amount_ranges
        stats['decline_rate_distribution'] = decline_rate_ranges
...
def preview_filter():
    ...
    try:
        ...
        # 计算统计信息
        if urls_data:
            decline_amounts = [item['decline_amount'] for item in urls_data]
            decline_rates = [item['decline_rate'] for item in urls_data]
            
            preview_stats = {
                'filtered_count': len(urls_data),
            ...
            preview_stats = {
                'filtered_count': 0,
                'decline_amount_range': None,
                'decline_rate_range': None
            }
    ...
    try:
        ...
        if query_progress_data.get('status') == 'finished':
            return jsonify({
                ...
                'total_count': len(query_progress_data.get('results', [])),
                'query_type': 'analysis',
                'status': 'finished'
            })
...
def get_cache_stats():
    ...
    try:
        with cache_lock:
            ...
            # 计算缓存命中率（基于API调用统计）
            total_api_calls = config_manager.get_api_usage_info().get('total_calls', 0)
            cache_hits = total_api_calls  # 这里需要实际统计
            ...
            # 模拟平均响应时间统计
            avg_response_time = 800  # 可以从实际调用中统计
            
            return jsonify({
                'success': True,
                'cache_items': total_cache_items,
                'cache_hit_rate': min(100, (cache_hits / max(total_api_calls, 1)) * 100),
